...
function Advantages() {
  ...
  return (
    <AdvantagesContainer>
      <AdvantagesTitle>{t('common:Advantages')}</AdvantagesTitle>
      <AdvantagesSubtitle color={primaryColor}>
        {t('common:We could not fit all of them here')}
      </AdvantagesSubtitle>
      <AdvantagesWrap>
        <Advantage>
          <Barber color={primaryColor} height={70} />
          <Heading3>{t('common:Professional barbers')}</Heading3>
          <Text>{t('common:Barbers who will change your hairstyle and yourself')}</Text>
        </Advantage>
        <Advantage>
          <Equipment color={primaryColor} height={70} />
          <Heading3>{t('common:Best equipment')}</Heading3>
          <Text>{t('common:We use cosmetics and tools that are known all over the world')}</Text>
        </Advantage>
        <Advantage>
          <BarbershopLocation color={primaryColor} height={70} />
          <Heading3>{t('common:Suitable location')}</Heading3>
          <Text>{t('common:We are in the heart of Warsaw, next to the Royal Łazienki Park')}</Text>
        </Advantage>
      </AdvantagesWrap>
    </AdvantagesContainer>
  );
